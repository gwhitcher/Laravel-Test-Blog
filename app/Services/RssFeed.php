...
class RssFeed
{
    ...
    {
        /* Uncomment to show cached version
    ...
    protected function buildRssData()
    {
        ...
        foreach ($posts as $post) {
            ...
            $item
                ...
                ->url(config('blog.base_url').'/blog/'.$post->slug)
                ->pubDate($post->published_at->timestamp)
                ->guid(config('blog.base_url').'/blog/'.$post->slug, true)
                ->appendTo($channel);
        }
        ...
        $feed = str_replace(
            '<channel>',
            '<channel>'."\n".'    <atom:link href="'.config('blog.base_url').
